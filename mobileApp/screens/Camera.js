...
import { AppRegistry, StyleSheet, Text, TouchableOpacity, View, TouchableHighlight, Dimensions, Alert , Platform, } from 'react-native';
import {Camera,Permissions,FileSystem,Constants,ImageManipulator,Gyroscope,} from 'expo';
...
export default class CameraExample extends React.Component {
  ...
  //...
    ...
    takePicture =  () => {
      ...
      if (this.camera) {
        this.camera.takePictureAsync({ onPictureSaved: this.onPictureSaved, skipProcessing:false,quality:1,} ); 
    }
    ...
    gohomewithdata = (uri) =>{
      ...
      this.props.navigation.navigate('Home',{takenphoto_uri:uri});
    ...
    onPictureSaved = async photo => {  
      ...
      let photo_urim = `${FileSystem.documentDirectory}photos/${Date.now()}.jpg`;       
      await FileSystem.moveAsync({        
        from: photo.uri,        
        to: photo_urim,
      });
      this.setState({ newPhotos: true });      
      this.gohomewithdata(photo_urim);
    ...
    <View
      ...
      <TouchableOpacity style={styles.toggleButton} onPress={this.toggleMoreOptions}>
        <MaterialCommunityIcons name={Platform.OS === 'ios' ? 'resize' : 'resize'} size={30} color="white"/>
      </TouchableOpacity>
    </View>
  ...
  renderBottomBar = () =>
    <View      style={styles.bottomBar}>
      ...
      <TouchableOpacity style={styles.bottomButton} onPress={this.gotoHome}>
            <Ionicons name={Platform.OS === 'ios' ? 'ios-home' : 'md-home'} size={25} color="white" />
      </TouchableOpacity>
      <View style={{ flex: 0.6 }}>
        <TouchableOpacity
...
gotoHome = () =>{  
  this.props.navigation.navigate('Home',{takenphoto_uri:null});
};
